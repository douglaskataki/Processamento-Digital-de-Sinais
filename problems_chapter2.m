...
histogram(x3);
%%
% 4) x4(n) = sum(k=1:4)yk(n), yk(n) é uma sequencia independente das outras
% com distribuição uniforme em [-0.5,0.5]
N4 = 1:10000;
y = zeros(1,length(N4));
for i=1:4
...
histogram(y,nbins);
% Distribuição Normal. Lembrando que existe uma função do matlab que
% encontra uma distribuição mais próxima do histograma (histfit).
%% P2.3
% Exemplo do livro:
% xtilde = x'*ones(1,P);
% xtilde = xtilde(:);
...
xlabel('n');
ylabel('imag(y(n))');
% Como o valor de w0 = 0.1*pi, então f0 = 0.1/2. Ou seja, 1/20 e isso nos
% dá K = 1 e N = 20. Para N = 200 amostras, então K = 10. Onde conseguimos
% nos gráficos que temos 10 períodos das partes reias e imaginárias de
% y(n).
%%
% 3)
n = -20:20;
...
xlabel('n');
ylabel('y(n)');
% temos um período igual ao mostrado anteriormente.
%% P2.6
% 1)
n1 = 0:9;
x1 = 0:9;
...
title('M=4');   
ylabel('y(n)');
% A função está correta, porém devido a aproximações feitas no matlab o
% valor é dado como diferente de zero. Mas pela escala de 1e-14, podemos
% verificar que de fato é zero o seu valor.
...
%% P2.10
% y(n) = x(n)+a*x(n-k)
% 1) Demonstraçao fora do escopo do Matlab
% 2) 
nx = 0:200;
